...
import type { UserState } from "@/lib/userState";
import {
  saveGroupHoldings,
  clearGroupHoldings as clearGroupHoldingsAction,
  saveExcluded,
  saveForceIncluded,
  savePortfolioValue,
  saveRhHoldings,
...
function PlaidConnectButton({
  ...
  const { open, ready } = usePlaidLink({
    ...
    onSuccess: async (publicToken) => {
      ...
      setError(null);
      try {
        // Server stores the access_token in Supabase; only the mapped
        // holdings come back.
...
function RobinhoodInput({
  ...
  holdings,
  setHoldings,
  initialPlaidConnected,
  initialPlaidLastRefreshedAt,
}: {
  portfolioValue: string;
  setPortfolioValue: (v: string) => void;
  holdings: RobinhoodHolding[];
  setHoldings: (h: RobinhoodHolding[]) => void;
  initialPlaidConnected: boolean;
  initialPlaidLastRefreshedAt: number | null;
}) {
  ...
  const [plaidConnected, setPlaidConnected] = useState(initialPlaidConnected);
  const [plaidRefreshing, setPlaidRefreshing] = useState(false);
  const [plaidError, setPlaidError] = useState<string | null>(null);
  const [plaidLastRefreshedAt, setPlaidLastRefreshedAt] = useState<
    number | null
  >(initialPlaidLastRefreshedAt);
  ...
  const refreshFromPlaid = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // 400 with "Plaid not connected" means the server already
        // dropped the row — reflect that in the UI.
  ...
  // On mount, if we know from the initial server state that Plaid is
  // connected, refresh holdings so the user sees current data.
  const bootstrapped = useRef(false);
  useEffect(() => {
    if (bootstrapped.current) return;
    bootstrapped.current = true;
    if (initialPlaidConnected) {
      refreshFromPlaid();
    }
  }, [initialPlaidConnected, refreshFromPlaid]);
...
// Hook: debounce a value so we don't fire a server action on every keystroke.
...
export default function HomeClient({
  ...
  const [step, setStep] = useState(initialState.groupHoldings.length > 0 ? 2 : 1);
  ...
  // Server-action wrappers — fire in transitions so UI stays responsive,
  // but errors surface to the console so we know if a write failed.
  const persist = useCallback(
    (fn: () => Promise<{ ok: boolean; error?: string }>) => {
      startTransition(async () => {
        const res = await fn();
        if (!res.ok && res.error) {
          console.error("Persist failed:", res.error);
        }
      });
    },
    []
  ...
  const handleGroupParsed = useCallback(
    (holdings: GroupHolding[]) => {
      ...
      setStep(2);
      persist(() => saveGroupHoldings(holdings));
    },
    [persist]
  );
  ...
  const toggleForceIncluded = useCallback(
    (cleanTicker: string) => {
      setForceIncluded((prev) => {
        const next = new Set(prev);
        if (next.has(cleanTicker)) next.delete(cleanTicker);
        else next.add(cleanTicker);
        persist(() => saveForceIncluded([...next]));
        return next;
      });
    },
    [persist]
  ...
  const clearGroupHoldings = useCallback(() => {
    ...
    setRhHoldings([]);
    setStep(1);
    persist(() => clearGroupHoldingsAction());
  }, [persist]);

  // Debounced writes for the inputs that change rapidly.
  const debouncedPortfolioValue = useDebounced(portfolioValue, 500);
  const portfolioInitial = useRef(true);
  useEffect(() => {
    if (portfolioInitial.current) {
      portfolioInitial.current = false;
      return;
    }
    persist(() => savePortfolioValue(debouncedPortfolioValue));
  }, [debouncedPortfolioValue, persist]);

  // rhHoldings change either from typed manual entry or from a Plaid
  // refresh — both should sync. Debounce keeps the manual case sane.
  const debouncedRhHoldings = useDebounced(rhHoldings, 500);
  const rhInitial = useRef(true);
  useEffect(() => {
    if (rhInitial.current) {
      rhInitial.current = false;
      return;
    }
    persist(() => saveRhHoldings(debouncedRhHoldings));
  }, [debouncedRhHoldings, persist]);
      ...
      <header className="border-b border-card-border bg-card/50">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {email ? (
              <span
                className="text-foreground/40 text-xs hidden sm:inline truncate max-w-[12rem]"
                title={email}
              >
                {email}
              </span>
            ...
            >
              <button
                type="submit"
                disabled={signingOut}
                className="text-foreground/50 text-xs font-semibold hover:text-foreground disabled:opacity-40"
              >
          ...
          <section className="bg-card rounded-2xl border border-card-border p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <RobinhoodInput
                ...
                initialPlaidConnected={initialState.plaidConnected}
                initialPlaidLastRefreshedAt={initialState.plaidLastRefreshedAt}
              />
              <ExclusionManager
